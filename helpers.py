...
# time_end = time.time()
# print("Time butter_bandpass_filter = ", time_end - time_start)

def removeArtifacts(data_input, events_input):
  """
  data = []
  events = []
  u = 0
  ...
  for i in data_input:
    ...
    for x in i:
      ...
      while c < 2:
        if c == 0:
          if x[c] == 9999999999999999999999999999999999999999999:
  ...
  return data_input, events_input
...
def extremePointsCorrelationMain(data, scaler):
  ...
  print("Wie lang ist denn data eig? ", len(data))

  minima_res = []
  maxima_res = []

  for epoch in data:
    minima, maxima = findLocalExtremesMain(epoch, scaler)
    minima_res.append(minima)
    maxima_res.append(maxima)
...
def getFrequencies(min, max, data):
  ...
  def getFrequenciesBackup(min, max, data):
    corr = []
    corr_tapered = []
    freqs = []
    i = min
...
def getFrequenciesPredefined_doesnotwork(data):
  corr_res = []
  corr = []
  monitor("DATA attention getFrequenciesPredefined", data)

  for epoch in data:

    corr.append(getFeatures(3, 6, epoch))
    corr.append(getFeatures(6, 14, epoch))
    corr.append(getFeatures(15, 30, epoch))
    corr.append(getFeatures(30, 45, epoch))

    corr_res.append(corr)

  cores_real_numbers = []
  for epoch in corr_res:
    for frequency in epoch:
      for sensor in np.transpose(frequency):
        for attribute in np.transpose(sensor):
          cores_real_numbers.append(attribute)
...
def getFrequenciesPredefined(data):
  corr = []
  monitor("DATA attention dudeldadeldai", data)

  corr.append(getFeatures(3, 6, data))
  corr.append(getFeatures(6, 14, data))
  corr.append(getFeatures(15, 30, data))
  corr.append(getFeatures(30, 45, data))
...
def concatenateFeatures(cores_real_numbers, mini, maxi):
    ...
    print("VISITE 3: ", len(np.transpose(X_reduced_res)), len(np.transpose(X_reduced_res)[0]))
...
def concatenateFeaturesMain(cores_real_numbers, mini, maxi, X_indices_real):
    ...
    for epoch in cores_real_numbers:
      X_predict = epoch.tolist()
    
      for i in mini[counter]:
          for x in i:
              X_predict.append(x)
      for i in maxi[counter]:
          for x in i:
              X_predict.append(x)

      counter += 1
      X_predict_res.append(X_predict)
            
      print("ConcatenateFeaturesMain: ", len(X_predict), len(cores_real_numbers), len(mini), len(maxi))

    # Reduce Features
    X_reduced_res_real = []
    for epoch in X_predict_res:
      print("ConcatenateFeaturesMain X_predict_res epoch", len(epoch))
      X_reduced_res_real_row = []
      c = 0
      for x in epoch:
          if c in X_indices_real:
...
def monitor(title, alist):
    print("---")
    print("Monitoring", title)
    a = alist
    addon = ""
    ...
    try:
        if len(a) > 0:
            while len(a) > 0:
                  print("length of ", title, addon, ": ", len(a))
                  a = a[0]
                  addon += "[0]"
    except:
      print("---")
